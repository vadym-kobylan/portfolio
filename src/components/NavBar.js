...
const CustomMobileLink = ({ href, title, className = '', toggle }) => {
  ...
  return (
    <button
      ...
      {title}
      <span
        className={`h-[1px] inline-block bg-light  absolute left-0 -bottom-0.5 group-hover:w-full trasition-[width] ease duration-300 dark:bg-dark ${
          router.asPath === href ? 'w-full' : 'w-0'
        }`}>
        &nbsp;
      </span>
...
const useOutsideAlerter = (wrapperRef, closeRef, setIsOpen) => {
  useEffect(() => {
    /**
      ...
      bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32">
          ...
          <nav className="flex items-center justify-center flex-wrap mt-7">
            <motion.a
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.9 }}
              className="w-6 mr-3 flex-grow-1 sm:mx-1">
              <InstagramMobileIcon />
            </motion.a>
            <motion.a
              href="https://github.com/vadym-kobylan"
